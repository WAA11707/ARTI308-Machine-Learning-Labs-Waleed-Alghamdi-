<div align="center">

# ARTI 308: Machine Learning Portfolio

**Spring 2026 | Imam Abdulrahman Bin Faisal University**

[![Student](https://img.shields.io/badge/Student-Waleed%20Adel%20Alghamdi-blue?logo=google-scholar&logoColor=white)](#)
[![ID](https://img.shields.io/badge/ID-2240006073-red)](#)
[![University](https://img.shields.io/badge/College-CCSIT%20%40%20IAU-green?logo=school&logoColor=white)](https://www.iau.edu.sa/en)

</div>

---

## 📘 Course Information

- **Course:** ARTI 308 - Machine Learning
- **Semester:** Spring 2026
- **College:** College of Computer Science and Information Technology (CCSIT)
- **University:** Imam Abdulrahman Bin Faisal University (IAU)

---

## 📂 Laboratory Submissions

| Lab | Title | Description | Status |
| :--- | :--- | :--- | :--- |
| **[Lab 1](./LAB-1)** | **Environment Setup** | Setting up Python, Jupyter, and ML libraries. | ✅ Completed |
| **[Lab 2](./LAB-2)** | **Problem Formulation** | Defining ML goals and initial data collection. | ✅ Completed |
| **[Lab 3](./LAB-3)** | **EDA** | Exploratory Data Analysis & Visualization. | ✅ Completed |
| **[Lab 4](./LAB-4)** | **Data Preprocessing** | Data cleaning, imputation, and PCA. | ✅ Completed |
| **[Lab 5](./LAB-5)** | **Feature Engineering** | Feature creation, peak hour rules, and selection. | ✅ Completed |
| **[Lab 6](./LAB-6)** | **Linear Regression** | Predicting yearly customer spending using the Ecommerce dataset. | ✅ Completed |
| **[Lab 7](./LAB-7)** | **Logistic Regression** | Predicting advertisement clicks using consumer behavior and demographic data. | ✅ Completed |

---

## 🔧 Tools & Technologies

The following tools and libraries are utilized throughout the course labs and projects:

| Category | Tools Used |
|:---|:---|
| **Language** | ![Python](https://img.shields.io/badge/Python-3.12%2B-blue?logo=python&logoColor=white) |
| **IDE** | ![VS Code](https://img.shields.io/badge/IDE-VS%20Code%20%2F%20Jupyter-007ACC?logo=visual-studio-code&logoColor=white) |
| **Data Manipulation** | ![Pandas](https://img.shields.io/badge/-Pandas-150458?logo=pandas&logoColor=white) ![NumPy](https://img.shields.io/badge/-NumPy-013243?logo=numpy&logoColor=white) |
| **Visualization** | ![Matplotlib](https://img.shields.io/badge/-Matplotlib-ffffff?logo=python&logoColor=black) ![Seaborn](https://img.shields.io/badge/-Seaborn-4c4cff?logo=python&logoColor=white) |
| **Machine Learning** | ![Scikit-Learn](https://img.shields.io/badge/-Scikit--Learn-F7931E?logo=scikit-learn&logoColor=white) |

---
*Last Updated: April 18th 2026*
